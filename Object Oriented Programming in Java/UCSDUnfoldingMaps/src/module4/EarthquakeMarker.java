...
public abstract class EarthquakeMarker extends SimplePointMarker
{
	...
	private void colorDetermine(PGraphics pg) {
		...
		if (depth >= THRESHOLD_DEEP) {
			pg.fill(255, 0, 0); // red for deep
		}
		else if (depth >= THRESHOLD_INTERMEDIATE){
			pg.fill(0, 0, 255); // blue for intermediate
		}
		else {
			pg.fill(255, 255, 0); // yellow for shallow
		}
	}
	
	private void lastDayDetermine(PGraphics pg, float x, float y) {
		// draw a cross if the earthquake occurred in the last day
		String age = this.getAge();
		if (age.equals("Past Day") || age.equals("Past Hour")) {
			pg.strokeWeight(2);
			int buffer = 2;
			pg.line(x-this.radius-buffer,y-this.radius-buffer,x+this.radius+buffer,y+this.radius+buffer);
			pg.line(x+this.radius+buffer,y-this.radius-buffer,x-this.radius-buffer,y+this.radius+buffer);
		}
